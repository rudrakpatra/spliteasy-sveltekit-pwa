...
export type CountryCode = typeof currencies[number]['countries'][number];
export type Currency = typeof currencies[number];

export const CURRENCY_ENUM: Record<CurrencyCode, Currency> = currencies.reduce((acc, currency) => {
    acc[currency.code] = currency;
    return acc;
}, {} as Record<CurrencyCode, Currency>);
export const DEFAULT_CURRENCY = CURRENCY_ENUM.USD;
export const CURRENCY_MAP = new Map<CurrencyCode, Currency>(currencies.map(currency => [currency.code as CurrencyCode, currency]));
export const COUNTRY_MAP = new Map<CountryCode, Currency>(currencies.flatMap(currency => currency.countries.map(country => [country, currency])));
