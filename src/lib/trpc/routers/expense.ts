...
export const expenseRouter = t.router({
    ...
    insert: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            const groupId = uuidSchema.decode(input.group_id);
            ...
            return await ctx.db.transaction(async (tx) => {
                ...
                input.splits.forEach((split) => {
                    const splitUserId = userIdSchema.decode(split.user_id);
                ...
                const splitsValues = input.splits.map((split) => ({
                    user_id: userIdSchema.decode(split.user_id),
                    currency: split.currency,
                    owes_amount: split.owes_amount as NumberString,
                    paid_amount: split.paid_amount as NumberString,
                    expense_id: newExpense.id,
                    group_id: groupId,
                    is_approved: userIdSchema.decode(split.user_id) === ctx.user.id,
                }));
    ...
    update: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            return await ctx.db.transaction(async (tx) => {
                // Verify membership
                await ensureGroupMember(tx, existing.group_id, ctx.user.id);
                ...
                // Replace splits
                await tx.delete(expenseSplits).where(eq(expenseSplits.expense_id, expenseId));

                const splitsToInsert = input.splits.map((s) => ({
                    expense_id: expenseId,
                    user_id: userIdSchema.decode(s.user_id),
                    group_id: groupId,
                    currency: s.currency,
                    owes_amount: s.owes_amount as NumberString,
                    paid_amount: s.paid_amount as NumberString,
                    is_approved: uuidSchema.decode(s.user_id) === ctx.user.id,
                    approval_time: uuidSchema.decode(s.user_id) === ctx.user.id ? sql`NOW()` : null,
                }));
    ...
    delete: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const [row] = await ctx.db
                .select({ groupId: expenses.group_id })
                .from(expenses)
                .where(eq(expenses.id, expenseId))
                .limit(1);
            ...
            const deleted = await ctx.db
                ...
                .where(
                    and(
                        ...
                        sqlIsGroupMember(expenses.group_id, ctx.user.id),
                        sql`NOT EXISTS (
    ...
    approve: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const updated = await ctx.db
                ...
                .set({
                    is_approved: true,
                    approval_time: sql`COALESCE(${expenseSplits.approval_time}, NOW())`,
                })
                .where(
                    and(
                        eq(expenseSplits.expense_id, expenseId),
                        eq(expenseSplits.user_id, ctx.user.id),
                        sqlIsGroupMember(expenses.group_id, ctx.user.id),
                    ),
                )
                .returning({
                    group_id: sql<string>`(SELECT e.group_id FROM expenses e WHERE e.id = ${expenseSplits}.expense_id LIMIT 1)`,
                    approved_count: sql<number>`(SELECT count(*) FROM expense_splits es WHERE es.expense_id = ${expenseSplits}.expense_id AND es.is_approved = true)`,
                    total_count: sql<number>`(SELECT count(*) FROM expense_splits es WHERE es.expense_id = ${expenseSplits}.expense_id)`,
                });
            ...
            const { group_id: groupId, approved_count: approvedCount, total_count: totalCount } = updated[0];
            ...
            return {
                ...
                approvedCount,
                totalCount,
                isFullyApproved: Number(approvedCount) === Number(totalCount),
            };
        }),
    ...
    getById: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
          ...
          `,
            ...
            if (!splits.find((s) => s.user_id === ctx.user.id)) {
                throw new TRPCError({
                    code: 'FORBIDDEN',
                    message: 'Unauthorized',
                });
